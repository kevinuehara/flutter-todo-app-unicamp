...
import 'package:flutter_todoapp/data/local/remote_database.dart';
...
class TaskBloc extends Bloc<TaskEvent, TaskState> {
  ...
  Stream<TaskState> mapEventToState(TaskEvent event) async* {
    if (event is FetchTaskList) {
      List<TaskDB> tasks = await DatabaseRemoteServer.helper.getTasks();
      yield TaskStateLoaded(tasks);
    } else if (event is UpdateTaskList) {
      await DatabaseRemoteServer.helper.updateTask(event.id, event.task);
      List<TaskDB> tasks = await DatabaseRemoteServer.helper.getTasks();
      yield TaskStateUpdated(tasks);
    } else if (event is CreateTaskEvent) {
      await DatabaseRemoteServer.helper.insertTask(event.task);
      yield CreateTaskState(task: event.task);
    } else if (event is UpdateTaskEvent) {
      await DatabaseRemoteServer.helper.updateTask(event.id, event.task);
      yield UpdateTaskState(task: event.task);
    } else if (event is DeleteTaskEvent) {
      await DatabaseRemoteServer.helper.deleteTask(event.id);
